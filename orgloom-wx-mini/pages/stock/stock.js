...
Page({  
  data: {
    ...
    ecTimeShareLine: {
      onInit: function (canvas, width, height) {
        ...
        barChart.setOption(getTimeShareLineOption());
        return barChart;
      }
    },
    ecScatter: {
    ...
    ecTimeShareBar: {
      onInit: function (canvas, width, height) {
        ...
        canvas.setChart(scatterChart);
        scatterChart.setOption(getTimeShareBarOption());
    ...
    ecKLineCandle: {      
      onInit: function (canvas, width, height) {
        ...
        canvas.setChart(scatterChart);
        scatterChart.setOption(getKLineCandleOption());
    ...
    modal: {
      detailMoreModalHidden: true
    },
    tab: {
      timeShareSelected: true,
      kLineSelected: false,
      dayKLineSelected: false,
      weekKLineSelected: false,
      monthKLineSelected: false,
      yearKLineSelected: false,
    },
    kLineTab: {
      news_selected: false,
  ...
  async isFollow(id) {
        ...
        console.log("getIsFollowed:"+JSON.stringify(json));

        if (json.code == 1000) {
        this.setData({
          isFollowed: json.data
        });
  ...
  async getCompany(id) {
    const json = await api.detailCompany({
      query: {
        id:id
    }
    });

  //  console.log("getDetailCompany:"+JSON.stringify(json));
   ...
   */
   onShareAppMessage: function () {
    return {
      title:'关注'+this.data.stockName+' 查看更多周边资讯信息'+',赶快点击加入关注吧',
      path: '/pages/stock/stock?name=' + this.data.stockName + '&id=' + this.data.id,
      imageUrl:"../../images/bonus01.png",
    }
  },
   /**
   * 页面上拉触底事件的处理函数
 ...
 //事件处理函数
  async  tapItem (e) {
      console.log('---------tree----click----parent--index接收到的itemid: ' + JSON.stringify(e.detail));
      let list=[];
      if(e.detail.path.toString().indexOf(",")>-1){
       list =  e.detail.path.split(",");
      } else{
        list.push(e.detail.path.toString());
      }

    //  console.log("----tree----click----parent--index接收到list----",list);
      this.data.treeModelPart=JSON.parse(JSON.stringify(this.data.treeModel));



      await this.setTree( this.data.treeModelPart,list);

      console.log("-------modelPart--parent--ssss--", this.data.treeModelPart);


      this.setData({
        treeModelEnd:this.data.treeModelPart,
      },function() {
        let myComponent = this.selectComponent(e.detail.idFlag);
        myComponent.test();

       
      });
    
    //  console.log("-------modelPart--parent ----www--", this.data.treeModelPart);

    },



    async setTree(s,list){
      s.forEach((e) =>{
        if(list.includes(e.id.toString())){
          e.checked=true;
          if(e.children!=null&&e.children){
          this.setTree(e.children,list);
          } else{
            e.children=[];
          }
        }else{
  ...
  async getShareList(id) {
    const json = await api.listShare({
      query: {
        id:id
    }
    });

    console.log("getShareList:"+JSON.stringify(json));

    if (json.code == 200) {

      for(var i = 0; i < json.data.list.length; i++){
  
       json.data.list[i].createDate=json.data.list[i].createDate.replace('T', " ").slice(0,10);
  
       }

    this.setData({
      shareList: json.data.list
    });
  ...
  async getFreeList(id) {
    const json = await api.listFree({
      method:'post',
      query: {
        orgId:id
    }
  ...
  //  console.log("getFreeList:"+JSON.stringify(json));
  ...
  async getOrgList(id) {
    const json = await api.listOrgTree({
      method:'post',
      query: {
        orgId:id
    }
  ...
  //  console.log("listOrgTree:"+JSON.stringify(json));
    ...
    if (json.code == 1000) {
     ...
     const treeDataTmp = await util.deepTree(json.data);
     const treeDataTmpPath = await util.setPath(treeDataTmp);
     const treeDataTmpPathCount=await util.setChildrenCount(treeDataTmpPath)

     this.data.treeModel=JSON.parse(JSON.stringify(treeDataTmpPathCount));



          // this.setData({
          //   orgList: this.data.treeModel
          // });

          let list=[];
          if(treeDataTmpPathCount.length>0)
          {
          list.push(treeDataTmpPathCount[0].id.toString());
          }
          await this.setTree(treeDataTmpPathCount,list);

          this.setData({
            treeModelEnd: treeDataTmpPathCount
          });
  //      console.log("-----this listOrgTree----",this.data.orgList);
  ...
  // 获取散点图
  async getShareListScatter(id) {
  
    let that=this;

    var data=[];
    var data2=[];
    const json = await api.listShareScatter({
      query: {
        id:id
    }
    });
    console.log("getShareListScatter:"+JSON.stringify(json));
    if (json.code == 200) {

      for(var i = 0; i < json.data.allScatter.length; i++){
  
        data.push([json.data.allScatter[i].years,json.data.allScatter[i].salary,json.data.allScatter[i].count]);
  
       }

       for(var i = 0; i < json.data.myScatter.length; i++){
  
        data2.push([json.data.allScatter[i].years,json.data.allScatter[i].salary,json.data.allScatter[i].count]);
  ...
  //  that.initGraph(that.data.scatterAll, that.data.scatterMy);
    
      this.scatterComponent.init(
        (canvas, width, height) => {
          // 初始化图表
          const barChart = echarts.init(canvas, null, {
           width: width,
          ...
          barChart.setOption(getScatterOption(that.data.scatterAll, that.data.scatterMy));
  ...
  (canvas, width, height) => {
    ...
    getScatterOption(chart, leftData, rightData);
    this.chart = chart;
    return chart;
  });
},
  ...
  /*
      this.scatterComponent.init(
        (canvas, width, height) => {
          ...
          barChart.setOption(this.getScatterOption());
          // 注意这里一定要返回 chart 实例，否则会影响事件处理等
          return barChart;
         });
...
*/
    ...
    clickAdd:function () {
      ...
      if(biz.checkLogin()){
    wx.navigateTo({
      url: '../editInfo/addInfo?companyId='+ this.data.companyDetail.id + '&companyName=' + this.data.companyDetail.name
    }) 
   } else{
    ...
    clickSay:function () {
      ...
      if(biz.checkLogin()){
    wx.navigateTo({
      url: '../writer/writer?companyId='+ this.data.companyDetail.id + '&companyName=' + this.data.companyDetail.name+ '&url=' + this.data.companyDetail.logoIcon
    }) 
   } else{
  ...
  clickHotDesc:function () {
    console.log("---hot---desc---");
    var page = this;
    this.setData({
      hot_show_selected: {
        isCard: true,
  ...
  async addFollow() {
    ...
    if(!this.data.isFollowed){
      ...
      wx.requestSubscribeMessage({
        tmplIds: ['-uEplpA5bcaN8hj-8pj4FBLHtJTlT39FKmrVLDKCwJ8'], //这里填入我们生成的模板id
        success(res) {
    ...
    const json = await api.addFollow({
      method:'post',
      query: {
      companyId: this.data.companyDetail.id,
      followed: !this.data.isFollowed
    }
    });

    if (json.code == 1000) {
     this.isFollow(this.data.companyDetail.id);
    } 
  }  else{
  ...
  //加减自选股票 
  addDelStock: function(){
    var page = this;
    //如果当前是自选，加自选
    if(!this.data.isSelected){
      wx.request({
        url: getApp().globalData.basicUrl + '/user/add',
        header: {
        ...
        data: {
          ...
          stockSinaCode: this.data.stockSinaCode
        },
        success(res){
          if (res.data.state == 0){
            page.setData({
    ...
    //减自选
      ...
      wx.request({
        url: getApp().globalData.basicUrl + '/user/del',
        header: {
          'content-type': 'application/json' // 默认值
        },
        method: 'post',
  ...
  initMarket: function(){
    var page = this;
    wx.request({
      url: getApp().globalData.basicUrl + '/stock/market/' + page.data.stockSinaCode,
      header: {
        'content-type': 'application/json' // 默认值
      },
      success(res) {
  ...
  initSelected: function(){
    var page = this;
    wx.request({
      url: getApp().globalData.basicUrl + '/user/is/selected',
      header: {
        'content-type': 'application/json' // 默认值
      },
      method: 'post',
  ...
  addFast: function() {
    wx.navigateTo({
      url: '../editInfo/addInfo?companyId='+ this.data.id + '&companyName=' + this.data.stockName,
    })
  },
  ...
  onLoad: function (options) {   
    ...
    // console.log("-----at into stock detail--"+options.id);

    this.setData({
      id:options.id,
      stockName: options.name,
    }); 
  //     this.scatterComponent = this.selectComponent('#mychart-scatter'); 
  //  this.initSelected();
  //  this.initMarket();
    this.getCompany(options.id);
    this.isFollow(options.id);
  //  this.getShareList(options.id);
  //  this.getShareListScatter(options.id);

    this.getFreeList(options.id);
    this.getOrgList(options.id);
    
 
  },
  //点击显示更多价格详情
  clickDetailMore: function () {
    this.setData({
      modal: { detailMoreModalHidden: false }
    })
  },
  clickDetailMoreClose: function () {
    this.setData({
      modal: { detailMoreModalHidden: true }
    })
  },
  //tab页选择
  clickTabTimeShare: function () {
    this.setData({
      tab: {
        timeShareSelected: true,
        kLineSelected: false,
  ...
  clickTabDayKline: function () {
    this.setData({
      tab: {
        timeShareSelected: false,
        kLineSelected: true,
        dayKLineSelected: true,
        weekKLineSelected: false,
        monthKLineSelected: false,
        yearKLineSelected: false,
      }
  ...
  clickTabWeekKline: function () {
    this.setData({
      tab: {
        timeShareSelected: false,
        kLineSelected: true,
        dayKLineSelected: false,
        weekKLineSelected: true,
        monthKLineSelected: false,
        yearKLineSelected: false,
      }
    })
  },
  clickTabMonthKline: function () {
    this.setData({
      tab: {
        timeShareSelected: false,
        kLineSelected: true,
        dayKLineSelected: false,
        weekKLineSelected: false,
        monthKLineSelected: true,
        yearKLineSelected: false,
      }
    })
  },
  clickTabYearKline: function () {
    this.setData({
      tab: {
        timeShareSelected: false,
        kLineSelected: true,
        ...
        weekKLineSelected: false,
        monthKLineSelected: false,
        yearKLineSelected: true,
      }
    })
  },
  //点击新闻tab
  clickKLineNewsTab: function(){
    this.setData({
      kLineTab: {
        news_selected: true,
...
//分时图折线
function getTimeShareLineOption(){  
  return{   
    xAxis: [
      {        
        axisLine: {
    ...
    series: [
      {
        ...
        symbol: 'none',        
        color: ['black'],
        data: [8.12, 8.34, 8.20, 8.45, 8.93, 8.93, 8.93, 8.93]
      },
      {
        name: '平均价格',
        type: 'line',
        ...
        smooth: 0.2,
        color: ['#CDA77C'],
        data: [8.12, 8.20, 8.20, 8.31, 8.45, 8.45, 8.45, 8.45]
      }
    ]   
  }
...
//分时柱状图
function getTimeShareBarOption() {
  return {
    grid: {
      left: 0,
      top: 0,
...
function getTimeShare(step){
  var timeSerise = [];
  //上午
  for (var iHour = 9; iHour < 12; iHour ++){
    var minMax, minMin;
    if (iHour == 11) minMax = 30;
    else minMax = 60;
    if (iHour == 9) minMax = 30;
    else minMax = 0;
    for (var iMin = minMax; iMin < 60; iMin += step){
      timeSerise.push(iHour + ":" + (Array(2).join('0') + iMin).slice(-2));
    }
  }
  //下午
  for (var iHour = 13; iHour < 15; iHour++) {    
    for (var iMin = 0; iMin < 60; iMin += step) {
      timeSerise.push(iHour + ":" + (Array(2).join('0') + iMin).slice(-2));
    }
  }
  return timeSerise;
...
// 点状图设置
function getScatterOption(leftData,rightData) {
