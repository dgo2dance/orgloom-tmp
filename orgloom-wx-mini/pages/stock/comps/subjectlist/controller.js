...
async function getList(page,status) {
  console.log("---view  data---",view.data);
  var { companyId} = view.data;

  let that=this;
  
  console.log("---view  data--companyId-filter--0--",JSON.parse(JSON.stringify(view.data.filter)));
  console.log("---view  data--companyId-filter----",JSON.parse(JSON.stringify(view.data.ids)));
  const json =await apiNew.listPost({
    method:"post",
    query: {
              page:page,
              size:10,
