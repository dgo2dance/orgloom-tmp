...
function onClickSubmit() {
  ...
  */
  ...
  if (view.data.images.length > 0) {
    console.log("---view.data.images---"+JSON.stringify(view.data.images));
    handler = uploadImages(data, view.data.images,view.data.postId);
  }
  */
...
async function addPost() {
  const json =await apiNew.addSay({
    method:"post",
    query: {
      companyId:view.data.id,
      companyName:view.data.name,
      title: view.data.title,
      contents: view.data.content,
      typeClassify:0
      }
  });

  console.log("addPost:"+JSON.stringify(json));

  if (json.code == 1000) {
    console.log("add--post---post---");
    view.setData({postId:json.data.id});

    var handler = undefined
    if (view.data.images.length > 0) {
      console.log("---view.data.images---"+JSON.stringify(view.data.images));
      handler = await uploadImages(view.data.images,view.data.postId);
    } 
    // else{
      wx.showToast({
    ...
    wx.navigateTo({
      url: '/pages/stock/stock?id=' + view.data.id
    //  url: '/pages/stock/stock?name=' + stockName + '&id=' + companyid,
    });
...
async function uploadImages(images,postId) {

  console.log("---upload images----"+JSON.stringify(images));
  for (var i = 0; i < images.length; i++) {
  await upload(images[i],postId);
  }
  /*
  return new Promise((res, rej) => {
    var array = []
    for (var i = 0; i < images.length; i++) {
      array.push(upload(images[i]))
    }
    /*
    Promise.all(array).then(results => {
      console.log("---upload images-----huidiao------");
      var path = JSON.stringify(results)
      console.log("---upload images-----huidiao------"+path);
      data.media = {
        path: path,
...
async function upload(tempFilePaths,postId) {
 ...
 var msg2 = JSON.parse(json).data;
    ...
    setPicPostData(msg2,postId);

  }
 }

 async function setPicPostData(key,postId) {
  
        const json =await apiNew.addSayImage({
          method:"post",
          query: {
            sayId:postId,
            imageIcon:key,
        ...
        console.log("addSayImage:"+JSON.stringify(json));

        if (json.code == 1000) {
          console.log("add--post--Img-post---");
          /*
          view.setData({postId:json.data.id});
          wx.showToast({
            title: '发送成功', icon: 'none'
