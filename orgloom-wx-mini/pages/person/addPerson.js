...
async addOrgPerson() {
  ...
  var param= {
    ...
    introRemark:this.data.introRemark,
    verifyClassify:1,
    imageIcon:imageIcon,
    profileIcon:imageIcon,
    verifyRemark:this.data.verifyRemark
    }
  };

  if(this.data.parentId!=null&&this.data.parentId!=""&&this.data.parentId.length>0) {
    param.query.parentId=this.data.parentId
  }
  const json = await api.addOrgPerson(param
  );
  console.log("addOrgPerson:"+JSON.stringify(json));
  ...
  if(json.code == 1000){
      ...
      wx.navigateTo({
        url: '../stock/stock?name=' + this.data.companyName + '&id=' + this.data.companyId
      })
  } else{
      if(json.code== 1001){
 ...
 submit: function() {
  ...
  onLoad: function (options) {
    console.log("rrrrrr-add person------"+JSON.stringify(options));
    that = this;
  ...
  //  this.getFree(options.id);
    ...
    if(options.parentId!=undefined&&options.parentId!=null&&options.parentId!='undefined'&&options.parentId!="")
    console.log("-------set parentId---options-----sss--");
    this.setData({
