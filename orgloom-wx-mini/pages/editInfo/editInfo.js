...
//   station:this.data.stations[this.data.statIndex]
  //加入人员
async updateFree() {
  const json = await api.updateFree({
    method:"post",
    query: {
    id:this.data.free.id,
    companyId:this.data.free.companyId,
    companyName:this.data.free.companyName,
    // provinceRemark:this.data.multiArray[0][this.data.multiIndex[0]],
    // cityRemark:this.data.multiArray[1][this.data.multiIndex[1]],
    provinceRemark:this.data.province,
    cityRemark:this.data.city,
    sexClassify:this.data.tradeIndex,
    freeName:this.data.freeName,
    interestClassifys:this.data.show.join(','),
    goalsClassifys:this.data.you.join(','),
    wechat:this.data.wechat,
    verifyRemark:this.data.verifyRemark,
    introduceRemark:this.data.introduceRemark

    }
  });
  console.log("updateFree:"+JSON.stringify(json));

  if(json.code == 1000){
    

        wx.showToast({
                title: '加入成功，后台审核中，审核完成即可得到消息',
                icon: 'none',
        })
