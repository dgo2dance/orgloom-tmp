...
async function checkLoginS() {
  const json = await apiNew.checkLogin({
    method:"post",
    query: {
      }
  });

  console.log("checkLogin:"+JSON.stringify(json));
  ...
  return true
    ...
    console.log("----checkLoginS---checkLogin----");
...
function checkLogin() {
  ...
  if (wx.getStorageSync("userInfo") && wx.getStorageSync("userInfo").phone) {
    return true
  }
